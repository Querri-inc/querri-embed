import { createSessionHandler, createQuerriClient } from '../../integrations/nextjs.js';
...
describe('createSessionHandler (Next.js)', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('calls resolveParams with request when provided', async () => {
    mockGetSession.mockResolvedValue({ session_token: 'tok', expires_in: 3600, user_id: 'u_1' });
    ...
    expect(resolveParams).toHaveBeenCalledWith(request);
...
describe('createQuerriClient (Next.js)', () => {
